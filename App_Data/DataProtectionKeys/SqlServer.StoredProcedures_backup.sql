CREATE FUNCTION [vptucksh_Admin].[nop_splitstring_to_table]
(
    @string NVARCHAR(MAX),
    @delimiter CHAR(1)
)
...
CREATE FUNCTION [vptucksh_Admin].[nop_getnotnullnotempty]
(
...
CREATE FUNCTION [vptucksh_Admin].[nop_getprimarykey_indexname]
(
...
CREATE FUNCTION [vptucksh_Admin].[nop_padright]
(
    @source INT, 
    @symbol NVARCHAR(MAX), 
    @length INT
...
CREATE PROCEDURE [vptucksh_Admin].[ProductLoadAllPaged]
(
...
CREATE PROCEDURE [vptucksh_Admin].[ProductTagCountLoadAll]
(
	@StoreId int
)
AS
...
CREATE PROCEDURE [vptucksh_Admin].[FullText_IsSupported]
AS
BEGIN	
	EXEC('
...
CREATE PROCEDURE [vptucksh_Admin].[FullText_Enable]
AS
BEGIN
	--create catalog
	EXEC('
...
CREATE PROCEDURE [vptucksh_Admin].[FullText_Disable]
AS
BEGIN
	EXEC('
	--drop indexes
...
CREATE PROCEDURE [vptucksh_Admin].[LanguagePackImport]
(
	@LanguageId int,
	@XmlPackage xml,
...
CREATE PROCEDURE [vptucksh_Admin].[DeleteGuests]
(
...
CREATE PROCEDURE [vptucksh_Admin].[CategoryLoadAllPaged]
(
    @ShowHidden         BIT = 0,
